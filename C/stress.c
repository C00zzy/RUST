...
void handle_signal(int signal) { // Funcution for detecting kill signals.. This CLI app is only for posix systems.
...
int main() {
    ...
    if (signal(SIGINT, handle_signal) == SIG_ERR) { // For ctrl + c
    perror("Exiting!\n");
    free(threads);
    exit(EXIT_FAILURE);
  }

  if (signal(SIGTERM, handle_signal) == SIG_ERR) { // For manual kill like kill -9
    perror("Exiting!\n");
    free(threads);
    exit(EXIT_FAILURE);
  }
